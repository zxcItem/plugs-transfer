...
class Transfer extends Controller
{
    ...
    {
        $this->skey = 'plugin.transfer.config';
        $this->title = '代理提现配置';
        $this->_sysdata();
    }
    ...
    {
        $this->skey = 'plugin.transfer.wxpay';
        $this->title = '微信提现配置';
        $this->_sysdata();
    }
    ...
    {
        PluginUserTransfer::mQuery()->layTable(function () {
            $this->title = '用户提现管理';
    ...
    {
        if ($this->request->isGet()) {
            PluginUserTransfer::mForm('audit', 'code');
        } else {
            $data = $this->_vali([
            ...
            $map = ['code' => $data['code']];
            $find = PluginUserTransfer::mk()->where($map)->find();
            ...
            if (PluginUserTransfer::mk()->strict(false)->where($map)->update($data) !== false) {
                $this->success('操作成功');
            } else {
                $this->error('操作失败！');
            }
