...
namespace plugin\transfer\controller\api\auth;

use plugin\account\controller\api\Auth;
use plugin\transfer\model\PluginUserTransfer;
use plugin\wemall\service\UserRebate;
use plugin\transfer\service\UserTransfer;
use think\admin\extend\CodeExtend;

/**
 * 代理提现接口
 * @class Transfer
 * @package plugin\transfer\controller\api\auth
 */
class Transfer extends Auth
{
    ...
    public function add()
    {
        ...
        $count = PluginUserTransfer::mk()->where($map)->count();
        ...
        // 写入代理提现数据
        if (PluginUserTransfer::mk()->save($data)) {
    ...
    public function get()
    {
        $query = PluginUserTransfer::mQuery()->where(['unid' => $this->unid]);
        ...
        $this->success('获取提现成功', array_merge($result, [
            'total' => [
                ...
                '锁定' => $locks,
                '可提' => $usable,
                '上月' => PluginUserTransfer::mk()->where($map)->whereLike('date', date("Y-m-%", strtotime('-1 month')))->sum('amount'),
                '本月' => PluginUserTransfer::mk()->where($map)->whereLike('date', date("Y-m-%"))->sum('amount'),
                '全年' => PluginUserTransfer::mk()->where($map)->whereLike('date', date("Y-%"))->sum('amount'),
            ],
        ]));
    }
    ...
    public function cancel()
    {
        ...
        PluginUserTransfer::mk()->where($data)->whereIn('status', [1, 2, 3])->update([
    ...
    {
        $data = $this->_vali(['unid.value' => $this->unid, 'code.require' => '单号不能为空！']);
        PluginUserTransfer::mk()->where($data)->whereIn('status', [4])->update([
