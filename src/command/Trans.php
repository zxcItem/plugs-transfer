...
class Trans extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $model = PluginUserTransfer::mk()->where(['type' => 'wechat_wallet', 'status' => [3, 4]]);
        [$total, $count, $error, $changeNow] = [(clone $model)->count(), 0, 0, date('Y-m-d H:i:s')];
        /** @var PluginUserTransfer $item */
    ...
    /**
     * 通过商户转账到余额
     * @param PluginUserTransfer $model
     * @return array
     ...
     * @throws \think\admin\Exception
     */
    private function createTransferV3(PluginUserTransfer $model): array
    {
     ...
     * @throws \think\admin\Exception
     */
    private function queryTransferV3(PluginUserTransfer $model): void
    {
     ...
     */
    private function createTransferBank(PluginUserTransfer $model): array
    {
     ...
     * @deprecated 微信商户已不再提供此接口
     */
    private function createTransferWallet(PluginUserTransfer $model): array
    {
     ...
     * @deprecated 微信商户已不再提供此接口
     */
    private function queryTransferBank(PluginUserTransfer $model)
    {
     ...
     * @throws \think\admin\Exception
     */
    private function queryTransferWallet(PluginUserTransfer $model)
    {
     ...
     * @throws \think\admin\Exception
     */
    private function getConfig(PluginUserTransfer $model): array
    {
        $data = sysdata('plugin.transfer.wxpay');
